...
To perform the path trace, we need the source and destination IP addresses. To get this information, we display the IP addresses for all available devices and hosts, so that the user can choose two addresses from this list. We will use what we learned in previous sessions to create this IP list.

* In Part I, we used the `GET /network-device` API to create a list of network-device IP addresses. We use the same API to create a list of network device IP addresses for this lab. Our code for this lab will extract the value of the `managementIpAddress` attribute of each block of the response body and display those IP addresses to the user.

* From the `GET /host` API , we can extract the value of the `hostIp` attribute from each block of the response body and display those IP addresses to the user.
...
def get_host_and_device():
    ...
    try:
        ...
        print ("Something went wrong. We cannot get the host IP list")
    ...
    try:
        ...
        print ("Something went wrong! We cannot get the network-device IP list !")
...
if \_\_name\_\_ == "\_\_main\_\_": # Execute only if run as a script
    # We use the tabulate module here to print a nice table format. You can use the `pip` tool to install it on your local machine
    # The tabulate module is imported in apicem.py
    # For simplicity, just copy the source code into your working directory, without installing it
...
After displaying this list, we can add code that prompts the user to select source/destination IP addresses from the list. Because the user-prompt code is based on code that is similar to what we wrote in Part I, we won't show it here or discuss it separately.<br>
...
* C. When the `GET /flow-analysis/{flowAnalysisId}` status attribute is "COMPLETED" then the response from `GET /flow-analysis/{flowAnalysisId}` contains the result we want.
...
When the `status` attribute in the response from `GET /flow-analysis/{flowAnalysisId}` is `COMPLETED`, then that response contains all the information we need for the path trace.


#### Code -- The complete example application

The code listing below puts together the following APIs to create the simple application that is described in the objectives section of this Learning Lab.
...
def check_status(arg,arg1):
    ...
    while status != "COMPLETED":
        ...
        try:
            ...
            # Something is wrong
            print ("\nSomething went wrong while executing get /flow-analysis/{flowAnalysisId}")
    print("\n------ End of path trace ! ------")
...
if \_\_name\_\_ == "\_\_main\_\_": # Execute only if run as a script
    ...
    if len(nd_list) < 2:
        print ("We need at least two hosts or network-devices to perform a path trace!")
        sys.exit()
    ...
    s_ip = select_ip('=> Select a number for the source IP from the list above: ',nd_list,ip_idx) # ip_idx (=2) is the position of IP in the list
    d_ip = select_ip('=> Select a number for the destination IP from the list above: ',nd_list,ip_idx) # ip_idx (=2) is the position of IP in the list
    # Now that we have the source and destination IP addresses we can use them to POST /flow-analysis
    ...
    try:
       flowAnalysisId = response_json["response"]["flowAnalysisId"]
        ...
        print ("\n For some reason, we cannot get the flowAnalysisId")
        sys.exit()
...
1. We display a list of host and network device IP addresses.
...
4. We use `GET /flow-analysis/{flowAnalysisId}` to monitor the progress of `POST /flow-analysis`.
5. From the `request["status"]` attribute in the response from `GET /flow-analysis/flowAnalysisId` we can monitor the `POST /flow-analysis` request status.
6. When the `POST /flow-analysis` task is done, then from the "networkElementsInfo" attribute in the response from `GET /flow-analysis/flowAnalysisId`,  we can see that the routing path started from host 10.2.1.22,  reached the Branch-Access1 switch with IP=10.2.1.17, and finally arrived at the host with IP=10.1.12.20, which is the destination.
...
* C. We should get either `COMPLETED` or `FAIL` as the status from `GET /flow-analysis/{flowAnalysisId}` when the task of POST /flow-analysis is finished. If it cannot get the result in 20 seconds, the script will time out.
