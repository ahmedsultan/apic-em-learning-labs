...
So far, we've learned how to create a policy and push it to a network device. In this lab, we will preview what will be pushed to a network device before we create/apply policy. 
...
* Applications — Software programs or network signaling protocols that are used in your network. EasyQoS includes Cisco Network-Based Application Recognition, a second generation (NBAR2) application library containing approximately 1400 distinct applications and sub-applications. In this session we will create a custom application to be used for creating policy.
...
We used these steps to create policy by using APIC-EM APIs. These steps also apply to Policy preview.
...
Except for the new function `get\_file\_with\_id( )`, all functions are the same ones we used when creating a policy script - `lab5-1-post-policy.py` that we will discuss in next session. These functions are mainly used to provide parameters to populate a JSON object for `POST policy/preview`. <br>
The contents of the preview will be saved in a file. We will use the `get\_file\_with\_id( )` function to retrieve file content by using the `GET file/{fileId}` API.<br>
    ...
    while check_name:
        ...
        if p_name == "":
            ...
            try:
                ...
                print ("Something is wrong. We cannot get the policy information")
                sys.exit()
            for item in policy:
...
def select_relevance():
    ...
    while True:
        tag_num = input('=> Enter one of tje numbers above for policy Business Relevancy: ')
...
def select_app(ap):
    ...
    # Prompt the user to select an application in order to retrieve its id
...
def get_file_with_id(ap,id):
    ...
    try:
        ...
        print ("Something is wrong. We cannot get file information")
        sys.exit()  
    ...
    while loop:
        ...
        for item in preview: # Entire response from GET policy/preview
            for item1 in item["policies"]:
                if item1["policyName"] == policy_name: # Make sure the preview is created, or we won't see the policy name.
                    ...
                    if item["deviceConfigs"] != []:
                        for item2 in item["deviceConfigs"]:
                            if item2["status"] == "FAILURE":
                                ...
                                # Since is nothing to view, delete this preview
                            ...
                            if count > 30: # Timeout after ~ 30 seconds
                                ...
                                # Since there is nothing to view, delete this preview
        ...
        if not preview_created: # The policy name is not found in all existing policy previews, so the preview is not created
...
#### The following code snippet shows how we parse the response from the previous figure.<br>
...
=> Enter application name from above (default or custom,case-sensitive) to create the policy: devnet_app
...
This concludes the policy preview lab session. In the next lab session, we will discuss how to delete a Policy Tag, policy Tag Association, Application, and Policy after creating them.
...
If you can not obtain policy preview, it could be that there is a **policy conflict**. If you use the task id returned by ```POST policy/preview``` applied to ```GET task/<id>``` API you may get a response similar to the following:
...
* In the policy preview script, we delete the policy preview content after showing it to the user. If, for any reason, the policy preview hangs, we can use the `lab4-2-delete-policy-preview.py` script to delete it.
