...
* Applications — Software programs or network signaling protocols that are used in your network. EasyQoS includes Cisco Network Based Application Recognition, a second generation (NBAR2) application library containing approximately 1400 distinct applications and sub-applications. In this session you will create a custom application to be used for creating policy.
...
Using the ``POST /policy/tag`` API is straightforward.
...
To check if the policy tag is created we can use the `GET /policy/tag` API. (See `lab1-2-get-policy-tag.py`.) We should see something like the following:
